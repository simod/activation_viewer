...
(function(){
  ...
  // Load disaster_types, keywords, and regions
  module.load_disaster_types = function ($http, $rootScope, $location){
        var params = typeof FILTER_TYPE == 'undefined' ? {} : {'type': FILTER_TYPE};

        $http.get(DISASTER_TYPES_ENDPOINT, {params: params}).success(function(data){
            if($location.search().hasOwnProperty('disaster_type__slug__in')){
                data.objects = module.set_initial_filters_from_query(data.objects,
                    $location.search()['disaster_type__slug__in'], 'slug');
            }
            $rootScope.disaster_types = data.objects;
        });
    }
  ...
  */
  module.run(function($http, $rootScope, $location){
    ...
    */
    if ($('#disaster-type').length > 0){
       module.load_disaster_types($http, $rootScope, $location);
    }
